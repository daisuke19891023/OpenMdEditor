...
test.describe('File Operations', () => {
  ...
  test('should create a new file', async ({ page }) => {
    ...
    const currentDraftId = await page.evaluate(() =>
  ...
  test('should save a draft manually', async ({ page }) => {
    ...
    ).toContainText(`下書き "${testFileName}" を保存しました`);
  ...
  test('should autosave a draft after editing', async ({ page }) => {
    ...
    const initialDrafts = await page.evaluate(() =>
      localStorage.getItem('markdownDrafts')
    );
    ...
    const finalDraftsString = await page.evaluate(() =>
      localStorage.getItem('markdownDrafts')
    );
  ...
  test('should load and delete a draft from the toolbar', async ({ page }) => {
    ...
    await page.evaluate(
      ([d1Name, d1Content, d2Name, d2Content]) => {
        const drafts = {
          draft1: {
            id: 'draft1',
            fileName: d1Name,
    ...
    await page
      .locator('[role="toolbar"] button:has-text("開く")')
      .click({ force: true });
    ...
    await expect(
      page.locator('[data-sonner-toast][data-type="info"]')
    ).toContainText(`下書き "${draft2Name}" を読み込みました`);

    // Draft2の削除アイコンをクリックし、AlertDialogで確認
    const deleteIcon = dropdownMenu
    ...
    await expect(
      page.locator('[data-sonner-toast][data-type="success"]')
    ).toContainText(`下書き "${draft2Name}" を削除しました`);

    // Draft2が存在しないことを確認し、Draft1が存在する
    await expect(
      dropdownMenu.getByRole('menuitem', { name: draft2Name })
    ).not.toBeVisible();
    await expect(
      dropdownMenu.getByRole('menuitem', { name: draft1Name })
    ).toBeVisible();
